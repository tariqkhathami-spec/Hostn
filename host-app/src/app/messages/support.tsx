...
export default function SupportChatScreen() {
  ...
  const sendMutation = useMutation({
    mutationFn: (text: string) => hostService.sendSupportMessage(text),
    onMutate: async (text) => {
      ...
      await queryClient.cancelQueries({ queryKey: ['support-chat'] });
      const previousMessages = queryClient.getQueryData(['support-chat']);
    ...
    onError: (_err, _text, context) => {
      if (context?.previousMessages) {
        queryClient.setQueryData(['support-chat'], context.previousMessages);
      }
    },
    onSettled: () => {
