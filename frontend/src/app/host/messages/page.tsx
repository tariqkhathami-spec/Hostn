  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">{isAr ? '\u062A\u0648\u0627\u0635\u0644 \u0645\u0639 \u0636\u064A\u0648\u0641\u0643' : 'Communicate with your guests'}</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" style={{ height: 'calc(100vh - 260px)', minHeight: '500px' }}>
        <div className="grid grid-cols-1 md:grid-cols-3 h-full">
          {/* Conversation List */}
          <div className={`border-e border-gray-100 flex flex-col ${selectedId ? 'hidden md:flex' : 'flex'}`}>
            <div className="p-4 border-b border-gray-100">
              <div className="relative">
                ...
                <input type="text" placeholder={isAr ? '\u0628\u062D\u062B...' : 'Search...'} value={search} onChange={e => setSearch(e.target.value)}
                  className="w-full ltr:pl-9 ltr:pr-3 rtl:pr-9 rtl:pl-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400" />
              </div>
            </div>
              ...
              {loading ? (
                ...
                <div className="flex-1 flex items-center justify-center p-6 text-center">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">{isAr ? '\u0633\u062A\u0638\u0647\u0631 \u0647\u0646\u0627 \u0639\u0646\u062F\u0645\u0627 \u064A\u062A\u0648\u0627\u0635\u0644 \u0645\u0639\u0643 \u0636\u064A\u0641' : 'Conversations will appear when guests message you'}</p>
                  </div>
                </div>
              ) : filtered.map(conv => {
                ...
                return (
                  <button key={conv._id} onClick={() => selectConversation(conv._id)}
                    className={`w-full p-4 flex items-start gap-3 hover:bg-gray-50 transition text-start border-b border-gray-50 ${selectedId === conv._id ? 'bg-primary-50' : ''}`}>
                    <div className="w-10 h-10 rounded-full bg-gray-200 flex-shrink-0 overflow-hidden">
                      {other?.avatar ? (
                        <Image src={other.avatar} alt="" width={40} height={40} className="w-full h-full object-cover" unoptimized />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-0.5">
                        ...
                        {unread > 0 && <span className="ms-2 bg-primary-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center flex-shrink-0">{unread > 9 ? '9+' : unread}</span>}
                      </div>
                    </div>
                  </button>
                );
                ...
                <div className="p-4 border-b border-gray-100 flex items-center gap-3">
                  ...
                  <div className="w-9 h-9 rounded-full bg-gray-200 flex-shrink-0 overflow-hidden">
                    {getOtherParticipant(selectedConv!)?.avatar ? (
                      <Image src={getOtherParticipant(selectedConv!)!.avatar!} alt="" width={36} height={36} className="w-full h-full object-cover" unoptimized />
                    ) : (
                    ...
                    return (
                      <div key={msg._id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${
                          ...
                          : isMe ? 'bg-primary-500 text-white rounded-br-md rtl:rounded-br-2xl rtl:rounded-bl-md' : 'bg-white border border-gray-100 text-gray-800 rounded-bl-md rtl:rounded-bl-2xl rtl:rounded-br-md'
                        }`}>
