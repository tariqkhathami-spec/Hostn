...
/** Connect to the Socket.IO server. Safe to call multiple times. */
export async function connectSocket() {
  ...
  // ── Availability changes → invalidate property data ─────────────
  socket.on('availability:changed', (data) => {
    console.log('[SOCKET] availability:changed', data.propertyId);
    if (queryClientRef) {
      queryClientRef.invalidateQueries({ queryKey: ['property', data.propertyId] });
...
function invalidateBookingCaches() {
  ...
  queryClientRef.invalidateQueries({ queryKey: ['myBookings'] });
  queryClientRef.invalidateQueries({ queryKey: ['booking'] });
